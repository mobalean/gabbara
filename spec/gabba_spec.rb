...
describe Gabba::Gabba do
  ...
  describe "parameter checking" do
    it "must require GA account for page_view" do
      expect {Gabba::Gabba.new(nil, nil).page_view("thing", "thing")}.to raise_error(Gabba::GoogleAnalyticsSetupError)
    end
    it "must require GA domain for page_view" do
      expect {Gabba::Gabba.new("abs", nil).page_view("thing", "thing")}.to raise_error(Gabba::GoogleAnalyticsSetupError)
    end
    it "must require GA account for event" do
      expect {Gabba::Gabba.new(nil, nil).event("cat1", "act1", "lab1", "val1")}.to raise_error(Gabba::GoogleAnalyticsSetupError)
    end
    it "must require GA domain for event" do
      expect {Gabba::Gabba.new("abs", nil).event("cat1", "act1", "lab1", "val1")}.to raise_error(Gabba::GoogleAnalyticsSetupError)
    end
  end
  ...
  describe "when tracking page views from request" do
    before do
      ...
      @gabba = Gabba::Gabba.new("UC-123", :request => test_request, :utmn => "1009731272", :utmcc => '')
    end
